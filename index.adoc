...
----
<1> Accederemos a `req.params` para saber si se han pasado parámetros y a `req.params.id` para acceder al parámetro `id`
<2> Llamada a la función `findByIdAndDelete` de Mongoose, inspirada en la función `findOneAndDelete` de Mongo, y se le pasará como parámetro el `id` del libro a borrar
...
<3> Comprobamos en la función de callback si no se han producido errores
<4> Se comprueba si se han pasado valores para cada campo del documento comporbando los parámetros pasados
