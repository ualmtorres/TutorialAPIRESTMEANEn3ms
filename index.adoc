...
https://swagger.io/[Swagger] es un framework para la creación y documentación de APIs REST. Podemos usar Swagger desde cero para crear la especificación de la API y Swagger nos asistirá en la generación de documentación y generación del código de la API, así como de otras tareas de _testing_ para comprobar, por ejemplo, si existe código asociado a cada tipo de estado devuelto por cada operación de la API. Otra opción es usar Swagger para documentar una API ya existente. En nuestro caso usaremos esta última opción para documentar la API desarrollada en este tutorial.
...
Al usar la `live demo` del editor online de Swagger ya tendremos disponible el código fuente de la documentación de la API y su vista previa. Iremos modificando el código fuente de ejemplo para documentar nuestra propia API.
...
La documentación de ejemplo generada está creada en YAML donde se destaca lo siguiente:
...
* `host` donde se desplegaría la documetación generada
...
<8> Método que se encarga de implementar la operación. Util cuando se usa la generáción de código y las funciones de testing
...
Una vez creada la documentación, para pasarla al proyecto usaremos el paquete Node.js `swagger-ui-express`, que sirve la documentación de Swagger mediante Swagger UI en la ruta que definamos en nuestro proyecto.

Después exportaremos a un JSON local la documentación creada en el editor online de Swagger.

Por último, añadiremos el JSON al proyecto y modificaremos `app.js` para incluir el acceso a la documentación.
...
Para poder usar la documentación JSON en nuestro proyecto usaremos la versión JSON de la documentación Swagger creada anteriormente. En el editor online seleccionaremos `Export` | `Download API` | `JSON Resolved`.
...
### Incportación de la documentación como una ruta del proyecto

Incluir el código siguiente en `app.js` para ofreer la documentación de la API a través de Swagger UI.
...
El resultado debería ser el siguiente tras acceder a `http://localhost:3000/api-docs`
